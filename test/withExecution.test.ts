...
describe('withExecution', () => {
  ...
  it('should call onCatch handler', async () => {
  ...
  it('should call onExecution and onCatch handler', async () => {
    const callee = vi.fn(() => { throw new Error('error') });
    const onExecution = vi.fn((params) => ({ result: params.callee(...params.args) }));
    const onCatch = vi.fn((params) => ({ caught: params.caught }));
  ...
  it('should be possible to supress an error', async () => {
    const callee = vi.fn(() => { throw new Error('error') });
  ...
  it('should be possible to modify the result', async () => {
    const callee = vi.fn(() => 42);
  ...
  it('should be possible to overload the set method on a map with logging', async () => {
    const map = new Map();
