...
describe('withOnCatch', () => {
    ...
    it('should pass the arguments to the onCatch event handler', () => {
        const func = vi.fn();
        const onCatch = vi.fn();
        const funcWithOnCatch = withOnCatch(func, onCatch);
        funcWithOnCatch(1, 2, 3);
        expect(onCatch).toHaveBeenCalledWith({ caught: undefined, callee: func, args: [1, 2, 3], event: 'onCatch' });
    });

    it('should modify the caught error with the onCatch event handler', async () => {
        const onCatch = ({ caught }) => ({ caught: new Error('modified ' + caught.message) });
        const wrapped = withOnCatch(() => { throw new Error('error') }, onCatch as any);
        try {
            await wrapped();
        } catch (error) {
